  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (user.avatar_url && user.avatar_url.key) {
        user.avatar_url = await getSignedImage(s3Client, user.avatar_url.key);
      }
...
export const updateUser = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { first_name, last_name, bio, avatar_url, dropbox } =
    ...
    try {
      ...
      if (avatar_url) {
        ...
        if (req.file) {
          ...
          const key = `profile_images/${userId}.${ext}`;
      ...
      // Get signed URL for avatar if it exists
      if (updatedUser.avatar_url && updatedUser.avatar_url.key) {
        updatedUser.avatar_url = await getSignedImage(
          s3Client,
          updatedUser.avatar_url.key
        );
      }
